...
export function parseMarkdownFiles(
  settings: MarkdownReaderSettings,
  store: any,
) {
  // Walk the input dir recursively, get a list of all files
  return walkDir(settings.inputDir, { exclude: settings.inputExclude }).chain(
    (files) => {
      const mdre = /(.*)\.md$/;
        ...
        .map(({ file, match }) => {
          ...
          return (
            reader
              ...
              .chain((fileReader) =>
                Task.fromPromise(store.trigger("md-file-parsed", fileReader)),
              )
